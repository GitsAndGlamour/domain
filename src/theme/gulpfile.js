...
gulp.task('js', function() {
  gulp.src(['js/**/*.js', '!js/configurator.js'])
    .pipe(jshint())
...
gulp.task('themes', function(cb) {
  ...
  for (var color in config.themes) {
    for (var shine in config.shines) {
      ...
      out.push(gulp.src(['scss/_config.scss'])
...
gulp.task('scss', function () {
  gulp.src('scss/**/*.scss')
    .pipe(gulpif(config.allColors, sassThemes('scss/themes/_*.scss', generateNames())))
